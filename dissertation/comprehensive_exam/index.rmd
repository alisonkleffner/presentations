...
+ Sea ice is a barrier between atmosphere and ocean
+ Cracks, or leads, may form in the ice pack due to dynamic processes
  ...
  - Account for half of the heat flux between the ocean and atmosphere `r Citep(bib[[c("badgley_1961")]])`
+ Prior Ice Crack detection methods can generally be split into two categories: thermal and deformation. 
...
+ Sea Ice is tracked by NASA's RADARSTAT Geophysical Processor System (RGPS), which uses synthetic aperture radar (SAR) images to track the trajectory of points on Arctic Sea Ice.
+ Each grid cell vertex is assigned an identifier (gpid) that is tracked
+ Set of all tracked trajectories: 

$$\mathcal{T} = (g_1, ..., g_n)$$
where 
$$g_{k} = <s_{1,j},...,s_{t,j}>$$ and \textbf{$$s_{i,j}} = $(x_{i,j}, y_{i,j})$$
...
+ Deformation of a cell is determined by the motion of points (vertices of cells that make up a grid) `r Citep(bib[[c("peterson_evaluating_2011")]])`
  - RGPS uses SAR imagery to extract sea ice motion
...
+ Develop a ice crack detection method based solely on the movement of the ice sheet
  - Motivated by the visualization of the movement
  - Cluster trajectories based on movement, where the boundaries would be possible locations of ice cracks
+ Using the information gained from the clustering, developed a spatio-interpolation method to predict missing points along a trajectory 
...
+ Feautures:
  - Length travel in x/y between the minimum and maximum location ($x_{max} - x_{min}$ and $y_{max} - y_{min}$)
...
1. Create Underlying Process Grid $$ U_{d,c}(s,t) \sim GP(\mu_{d,c}, C_{d,c}(\theta)) $$
