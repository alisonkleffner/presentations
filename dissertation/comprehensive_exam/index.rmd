...
+ For our study region, n = 8811 ids, and T = 21 days
...
+ Use thermal channels of the Advanced Very High Resolution Radiometer (AVHRR) `r Citep(bib[[c("key_detectability_1993")]])` 
  - Heavily dependent on clear skies and thin ice
...
+ Combination of geographic location with time introduces new challenges in cluster, where a cluster is now determined based on spatial and temporal similarity $\\$ $\\$
+ Similarity Measures
...
+ Linear interpolation 
  ...
  - Performs best for linearly moving objects, or can potentially work well with non-linear data if sample in high frequency `r Citep(bib[[c("guo_improved_2021")]])`
...
+ Using information gained from clusters to identify spatio-temporal neighbors
  ...
  - Would expect a missing point  at that time to move similarly
+ Cluster By Week
  - Clustering sub-trajectories is beneficial as may lost information if clustering the whole trajectory.
+ Use the by Week clusters to find neighbors
...
+ Determining estimates of missing locations
  ...
  - The model will then adjust this location using it's known neighbors
...
+ Create Underlying Process Grid 
  ...
  - Initial grid was created and shifted seven times to represent seven days of movement.
...
where $U_{d,c}(s,t)$ is the displacement at location $s$ & time $t$ $\\$ for dimension $d$ ( $d$ =x, y), cluster $c$

]

**Note**: Only simulated 2 clusters for simplicity
...
+ Create Observed Grid
  - Movement of an observed point is determined by the value fo the nearest point of the underlying process for that day ( $g$ )
...
kableExtra::kable(result_data, booktabs = TRUE, caption = "RMSE for Interpolation Methods", col.names = c("Simulation", "X", "Y", "X", "Y", "X", "Y"), escape = FALSE,  table.attr = "style='width:80%;'") %>% 
...
kableExtra::kable(result_data3, booktabs = TRUE, caption = "Proportion of Intervals Contained Observed Value", col.names = c("Simulation", "X", "Y"), escape = FALSE, align = "c", table.attr = "style='width:80%;'") 
...
kableExtra::kable(result_data6, booktabs = TRUE, caption = "Proportion of Intervals Containing Observed Value by cluster for Week 2", col.names = c( "Cluster", "X", "Y"), escape = FALSE, align = "c", table.attr = "style='width:75%;'") %>% row_spec(1, color = 'black', background = 'yellow') 
...
print(bib[[c("schreyer_elastic_2006", "badgley_1961", "key_detectability_1993", "guo_improved_2021", "gpgp_pkg", "peterson_evaluating_2011", "bouillon_producing_2015", "rohrs_algorithm_2012", "kim_analyzing_2005", "kodinariya_2013")]], 
