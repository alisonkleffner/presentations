...
These plots include many design choices that hinder a users ability to accurately explore the relationship between the two variables. I will go more into detail with some of these decisions in the next couple of slides. This first map overlays the yield measurements with the trial design. One of the big issues with this plot is the overlapping of the yield measurements. It may be difficult to see in this presentation, but the yield measurements are stacked on top of each other, obscuring the color. This second plot separates out the trial design and yield plots, where instead of yield measurements, we show the processed (cleaned) data with yield polygons. In this plot it is difficult to assess the relationship due to the lack of spatial cues between the plots, so may not be comparing the same location on each plot. 
...
Plenty of other examples are found throughout the literature related to the studying of this relationship. For example in this first plot we have the applied nitrogen rates and yield measurements side-by-side. The nitrogen rates use a rainbow color scheme which is problematic for multiple reasons. One, the ends of the scale are visually close (red and violet), so its hard to differentiate between the ends of the scale. Second, this color scheme has no inherent ordering, when the magnitude of yield does. The second plot contains trial designs (two input trial - top two rows) and yield measurments for four fields. Not only are we asking a user to do a lot of work in comparing, the same color scheme was used for everything adding to the confusion. 
...
Now, going to take a deep dive into these critiques. First we look at different layouts for comparative graphs (comparing variables to derive a relationship). The pros and cons of each are reversed. The juxtaposed layout involves side-by-side graphs. A benefit is that these are easy to create and they tend to not have issues with visual clutter as the variables are separated out. However, a major drawback is that the comparative burden is placed on the user. Deriving relationships becomes even more difficult in the spatial setting, as the user has to accurately find the same location in each graph. So many areas where an error can be made. Superimposed graphs include multiple objects in the same coordinate system, where the major drawback is clutter (overlapping points obscuring information). However, they are useful when spatial location is a key component of the comparison as users can use perception rather than memory. 
...
**Extract Features**: Trajectories are complicated to work with, so we can extract features from the raw data that can summarize its movement `r Citep(bib[[c("climate-viz")]])` 
  ...
  + Example: TPoSTE, which created features based on the visualization of events to separate a boat’s trajectory into periods of fishing or sailing `r Citep(bib[[c("wu-fish-2022")]])`. 
...
$$(\hat{u}_{t-1}, \hat{v}_{t-1}) + (x_{t-1}, y_{t-1}) = (x_t, y_t)$$
...
$$(\hat{u}_{t-3}, \hat{v}_{t-3}) + (x_{t-3}, y_{t-3}) = (x_t, y_t)$$
