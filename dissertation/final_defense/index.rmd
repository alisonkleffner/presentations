...
i1 <- img_modal(src = "images/bb_2.png", alt = "Points used to Develop Bounding Box 2", other=list(width="60%"))
...
# Thermal Lead Detection Methods
...
# Deformation Lead Detection Methods
...
+ Deformation of a cell is determined by the motion of points `r Citep(bib[[c("peterson_evaluating_2011")]])`
    ...
    - Can find the size and orientation of the lead
+ Drawbacks
...
To use a model within an intersection, a spatial grid encompassing the sea ice is created. The centroid of the grid cells helped us obtain a starting estimation of the missing $g_j$. The underlying process should be smooth within an intersection, so the grid centroid provides a location near the true location which would have a similar value of the underlying process. 
