...
`r fa_i("image")` Overall Motivation

`r fa_i("line-chart")` Redesigning Yield Map Plots for Comprehension and Usability

`r fa_i("arrows-alt")` Visual Diagnostics for Trajectory Data

`r fa_i("space-shuttle")` Spatio-Temporal Model for Arctic Sea Ice

`r fa_i("exclamation")` Overall Conclusion
...
+ Example: Nitrogen Fertilizer
  - Nitrogen is an essential component of food production as is allows plants to photosynthesize efficiently `r Citep(bib[[c("MAHESWARI2017175")]])`  
...
**Data Intensive Farm Management (DIFM)**: Conduct On-Farm Precision Experiments to find economically optimal application rate to increase profit while reducing environmental impacts.


.center[
```{r results='asis', echo = F, include = T, cache = T, eval = TRUE}
i1 <- img_modal(src = "images/logo.png", alt = "Data Intensive Farm Management (DIFM) Project",other=list(width="30%"))
...
.pull-left[
**Step 1**: Design experiment using site-specific inputs
...
**How**: Machine learning models to learn how crop yield responds to different input application rates, field characteristics, and weather
  - Build trust in models

**One way to do this**: Visually explore the relationship between input application and yield
...
+ Multiple dots on top of one another
  ...
  - Visual cues, like color, becomes partially obstructed, reducing search efficiency `r Citep(bib[[c("BRAVO2004b", "BRAVO2004a")]])`
...
+ A mental image is relied on for comparison, as the user moves their eyes between images (shifting focus). 
...
+ Red-green color blindness
  - Experienced by approximately 8% of men and 0.5% of Northern European Ancestory `r Citep(bib[[c("wong2011color")]])`
...
.center[
```{r results='asis', echo = F, include = T, cache = T, eval = TRUE}
#i1 <- img_modal(src = "images/Attempt1.png", alt = "First Iteration", other=list(width="40%"))
i2 <- img_modal(src = "images/Attempt2-new.png", alt = "Second Iteration", other=list(width="40%"))
...
+ Yield Polygons
+ Transparency to show both at same time
+ Change Colors
...
+ Interactivity to connect the plots 
  - Hovering over a trial plot highlights the corresponding points in the scatterplot used in the correlation calculation.
...
Trajectories are considered the most complex form of data based on points, but are becoming more easily available `r Citep(bib[[c("kisilevich_spatio-temporal_nodate", "rinzivillo_visuallydriven_2008")]])`
...
+ Relevant features can also be determined using visualization and make the trajectories easier to work with in future analyses.
...
+ Sea ice insulates the warm ocean from the colder atmosphere `r Citep(bib[[c("peterson_evaluating_2011")]])`
...
+ The state of sea ice and understanding the dynamics driving changes provides information for weather prediction, climate and ocean models `r Citep(bib[[c("reiser_new_2020")]])` 
...
i1 <- img_modal(src = "images/Ice Chunk.png", alt = " Artic Sea Ice with Crack", other=list(width="30%"))
...
.center[
```{r ex-results,  results='asis', echo = F, include = T, cache = T, eval = TRUE}


i1 <- img_modal(src = "images/plot4.png", alt = "Example Lead Detection Results", other=list(width="20%"))
...
**Lead Detection**: Use bounding box features in K-Means clustering
...
**Finding Neighbors**: Use information gained from clusters to identify spatio-temporal neighbors
...
+ Cluster trajectories by week to find neighbors
  ...
  - Each member of an intersection are spatio-temporal neighbors as they are in a similar geographic region over time. 
...
Let's first begin by introducing a univariate process: $$\left\{H(s,t), (s,t) \in \mathcal{D} \in \mathcal{R}^3\right\}$$ where there are $n$ spatial locations at $t$ time points.

The process is assumed Gaussian, where the model can be rewritten as 
$$H_{{i,t}} \sim N(\eta_{i,t}, \sigma^2_e)$$ 
...
When working with point data, assuming a continuously indexed GF for $w_{i,t}$ is not a computationally efficient approach.
...
$$\eta_{i,t} = \alpha + \sum^{G}_{g=1}\tilde{A}_{itg}\tilde{w}$$ 
where $\tilde{A}_{itg}$ is a sparse precision matrix mapping the GMRF, $\tilde{w}$, from $n$ locations to the $G$ nodes in the triangulation
]
...
We can represent $u$ and $v$ by the joint process $$\left\{B(s,t), (s,t) \in \mathcal{D} \in \mathcal{R}^3\right\}$$

The process is assumed Gaussian, where the model can be rewritten as 
$$B_{{i,t}} \sim N(\eta_{i,t}, \sigma^2_e)$$ 
...
We developed a model within each intersection to account for the non-stationary aspect of our data using only the data from t − 1, t, and t + 1. 

Create a spatial grid encompassing the sea ice. 
  + Use the centroid of each grid cell to obtain estimated values for the underlying process
  + Use last known location to determine which grid cell to use as an estimate
...
+ We fit the spatial model with a time fixed effect 
  - Only using three days to develop our model, so there is probably not a significant amount of temporal
dependence.
  - Performs better for $y$, the already worse performing coordinate in the simulation study
...
A benefit of using a model-based approach is that we can determine the uncertainty of our estimate.
  ...
  - The intervals can then be used to find the proportion of intervals containing the true amount of movement during testing of our models
...
The amount of trajectory and multivariate spatial data collected has increased due to technological advances, where each data type has characteristics that make them complicated to work with.

This research presents work with each data type using environmental applications to address some of these complications.
  + Importance of design choices when developing plots the occupy the same spatial domain
  + Messy visualizations can be used for feature extraction
  + Developing a non-stationary spatio-temporal model to reconstruct a process causing items to move
...
Meaning, for $\left\{s_1,...,s_n\right\}$, $x$ = $(x_1,...,x_n)^T \sim MVN(0, \sigma^2_c\Sigma_{\theta_{i,q}})$

Can define $\Sigma_{\theta_{i,q}}$ as the Matern Covariance Function `r Citep(bib[[c("gpgp_pkg")]])`


$$\Sigma_{\theta_{i,q}} = \frac{1}{\Gamma(\lambda)2^{\lambda-1}}(\kappa||s_{i}-s_{q}||)^{\lambda} K_{\lambda}(\kappa||s_{i}-s_{q}||)$$ $\\$
where $\lambda$ is the smoothness parameter, $\kappa$ the scaling parameter, and $K_{\lambda}()$ the Bessel function 
...
Now, the covariance function is a Matern Space-Time, which is a separable covariance function (Wikle et al, 2019)

.center[
$\Sigma = \sigma^2_{c}C_{1}(\Delta_{iq})C_{2}(\Lambda_{tp})$ $\\$ where 
$C_{1}(\Delta_{iq}) = \frac{1}{\Gamma(\lambda)2^{\lambda-1}}(\kappa||s_{i}-s_{q}||)^{\lambda} K_{\lambda}(\kappa||s_{i}-s_{q}||)$  and $C_{2}(\Lambda_{tp})$ is an autoregressive order 1 (AR(1)) model with a temporal lag of $||t-p||$ 


where $\sigma^2_{c}$ is the variance component, $\lambda$ is the smoothness parameter, $\kappa$ the scaling parameter related to the range, and $K_{\lambda}()$ the Bessel function of the second kind.
]
...
+ Create Underlying Process Grid 
  ...
  - First generate a fine grid (40x40)
  - To simulated a covariance matrix, the initial grid over seven days was used in the Matern Space-Time covariance Function.
...
.center[
$H_{c}(s,t) \sim GP(\mu_{c}, C_{c}(\theta))$ $\\$

where $H_{c}(s,t)$ is the joint displacement at location $s$ & time $t$ $\\$ for cluster $c$
...
+ Create Observed Grid
  - Movement of an observed point is determined by the value of the nearest point of the underlying process for that day ($r$)
  - Obtained a week's worth of simulated data
  
.center[
  $(x_{t,j}, y_{t,j}) = (H_{t-1,c,r}, H_{t-1,c,r}) + (x_{t-1,j}, y_{t-1,j})$ $\\$
  where t=1,...,7 (time), j = 1,...,121 (id), $\\$ H is the underlying process value at $t-1$ for cluster ( $c$ ) and grid id ( $r$ )
]
...
i1 <- img_modal(src = "images/grid-combo.png", alt = "Underlying and Observed Grid Plotted Together", other=list(width="40%"))
...
i1 <- img_modal(src = "images/sim-traj.png", alt = "Simulated Trajectories for Each Simulation", other=list(width="90%"))
...
i1 <- img_modal(src = "images/sim-clust-results.png", alt = "Clusters for each Simulation at t=0 and t=7", other=list(width="80%"))
...
result_data <- data.frame(sim = c(1,2,3), X1 = c(0.160, 0.173, 0.375), Y1 = c(0.263, 0.281, 0.558), X2 = c(0.192, 0.172, 0.399), Y2 = c(0.307, 0.229, 0.598), X3 = c(0.167, 0.172, 0.303), Y3 = c(0.418, 0.480, 0.659), X4 = c(0.197, 0.201, 0.402), Y4 = c(0.599, 0.670, 1.050))
 
kableExtra::kable(result_data, booktabs = TRUE, caption = "RMSE for Interpolation Methods", col.names = c("Simulation", "X", "Y", "X", "Y", "X", "Y", "X", "Y"), escape = FALSE,  table.attr = "style='width:80%;'", align = "c") %>% 
  add_header_above(c("", "Linear" = 2, "Spatial Model" = 2, "ST Model" = 2, "Stationary Model" = 2))
...
result_data2 <- data.frame(sim = c(1, "", 2, "", 3, ""), Cluster = c(1,2,1,2,1,2), X1 = c(0.154, 0.163, 0.148, 0.185, 0.408, 0.304), Y1 = c(0.252, 0.269, 0.256, 0.293, 0.448, 0.721), X2 = c(0.159, 0.208, 0.137, 0.186, 0.315, 0.443), Y2 = c(0.305, 0.308, 0.195, 0.244, 0.737, 0.493), X3 = c(0.077, 0.201, 0.119, 0.192, 0.193, 0.354), Y3 = c(0.272, 0.481, 0.381, 0.522, 0.785, 0.569), X4 = c(0.182, 0.206, 0.164, 0.219, 0.33, 0.439), Y4 = c(0.667, 0.555, 0.774, 0.6, 1.290, 0.872))

kableExtra::kable(result_data2, booktabs = TRUE, caption = "RMSE for Interpolation Methods by cluster", col.names = c("Simulation", "Cluster", "X", "Y", "X", "Y", "X", "Y", "X", "Y"), escape = FALSE, align = "c") %>% add_header_above(c(" " = 2, "Linear" = 2, "Spatial Model" = 2, "ST Model" = 2, "Stationary" = 2))  
...
result_data3 <- data.frame(sim = c(1, 2, 3), x1 = c(0.61, 0.829, 0.579), y1 = c(0.597, 0.658, 0.697), x2 = c(0.716, 0.829, 0.817), y2 = c(0.313, 0.357, 0.507), x3 = c(0.615, 0.769, 0.654), y3 = c(0.205, 0.32, 0.256))

kableExtra::kable(result_data3, booktabs = TRUE, caption = "Coverage", col.names = c("Simulation", "X", "Y", "X", "Y", "X", "Y"), escape = FALSE, align = "c", table.attr = "style='width:80%;'")  %>% add_header_above(c(" " = 1, "Spatial Model" = 2, "ST Model" = 2, "Stationary" = 2))  
...
i1 <- img_modal(src = "images/int-examp.png", alt = "Comparison of Our Results to a Kinematic Crack Algorithm", other=list(width="80%"))
...
---
class:primary
# Existing Trajectory Clustering
...
---
class:primary
# Exisiting Spatio-Temporal Interpolation
...
+ Dynamic Spatio-Temporal Interpolation
  - Reconstruct spatio-temporal dynamics during interpolation
  - Mostly assume stationarity which is often inappropriate
  - Example: Optimal Interpolation models the covariance function of the spatio-temporal field dynamics. Then a linear combination of the observations results in an interpolated field, assuming stationarity `r Citep(bib["fablet_data-driven_2017", "ouala_neural_2018"])`
