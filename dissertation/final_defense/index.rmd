...
We can represent $u$ and $v$ by the joint process: $$\left\{H(s,t), (s,t) \in \mathcal{D} \in \mathcal{R}^3\right\}$$ where there are $n$ spatial locations at $t$ time points.
...
with $\alpha$ denoting the intercept and the realization of the latent ST Gaussian Field (GF) is represented by $w \sim BiGF(0,\Sigma)$. The covariance function $(\Sigma)$ is the separable Matern ST covariance function `r Citep(bib[[c("BLANGIARDO201333")]])`. 
...
**Using Model**: Create a spatial grid encompassing the sea ice. 
  + The underlying process should be smooth within an intersection, so should have a similar value of the underlying process. 
