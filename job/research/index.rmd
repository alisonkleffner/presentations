...
**Education**
+ BS in Applied Mathematics and Economics from Rockhurst University in 2018
+ M.S from University of Nebraska - Lincoln in 2020
+ PhD Candidate at the University of Nebraska at Lincoln with expected graduation in August 2023
...
+ Research Assistantship: Data Intensive Farm Management Project
+ Research Areas: Visualization, Spatio-Temporal Modeling, Environmental Applications
...
???
To begin, I wanted to share a little bit about myself. I completed my undergraduate at Rockhurst University in Kansas City, MO with a double major in Applied Mathematics and Economics. From there I have been in Lincoln, NE for the last 5 years working on my PhD in Statistics. During graduate school, I have gained experience by teaching UNL's introductory statistics course sequence, which uses simulation-based statistics. I have also in these courses taught different regression and basics of experimental design methods, like blocking and ANOVA. For the past year I have been a research assistant on the Data Intensive Farm Management Grant where I have collaborated with other researchers throughout the country. My work on this project will be the focus of my talk today. My research interests in general involve data visualization, environmental applications of statistics, and spatio-temporal modeling. I am currently working on a paper developing a non-stationary spatio-temporal model for Arctic sea ice, which I hope to submit for review to the Journal of Agricultural, Biological, and Environmental Statistics by the end of the year. What I am presenting to you today is also in the early stages of writing to submit for publication in the near future.
...
+ Cracks, or leads, may form in the sea ice due to dynamic processes
...
Briefly I wanted to show you all some work that I have been doing involving spatio-temporal modeling for environmental application. If you want to talk about this topic more, I would be happy to after the talk. We wanted to develop a non-stationary spatio-temporal model for the movement of Arctic Sea Ice, meaning a model for movement over time and space where the model has a non-constant mean and covariance function. Modeling Sea ice movement is important as sea ice prevents the warm air from the ocean from releasing into the cooler atmosphere. However, due to dynamic process, like ocean currents, cracks, otherwise known as leads, may be formed (Show Picture). Leads only account for about 1-2% of the surface of sea ice, but they account for half of the heat flux between the ocean and the atmosphere. Thus, it is important to model the sea ice and determine lead location for weather prediction, climate models, and ocean models. 
...
We wanted to only use the movement of the sea ice to develop our methods. This is because remote sensing data tends to have coarse resolution, so it does not capture lead dynamics very well. Our method is motivated from looking at the movement of the sea ice (show picture, point things out). We first start by clustering the regions of similar movement, where the cluster boundaries would be locations were leads form as it is the boundaries between movements (show cluster picture). The clusters then form the different non-stationary movements where different means and covariance parameters. Within each cluster we developed a spatio-temporal Gaussian process model to interpolate the missing data. In cluster picture, can see that the data is missing in spatial chunks, so hope to use information about movement from other members in it's cluster to help interpolate. Interpolating to have a completely gridded dataset is beneficial because can use this information to do additional calculations. Seems to compare well to previous crack detection methods (show picture) 
...
In my talk today I a going to start by motivating the importance of visualization. Then I will talk about how to make a good graph through the use of graphical perception.I will end with work for my dissertation on the redesigning of yield plots keeping in mind what we know about the creation of optimally perceptible graphs. I will then conclude with my future work. 
...
**What are graphics useful for?** `r Citep(bib[[c("unwin2020data")]])`
+ Data cleaning
...
To get started we are going first going to lay the foundation of graphics and why we want to use them. Data visualization has become a central tool in modern data science and statistics. Data visualization can be defined as the art of drawing graphical charts to display data. They are useful for data cleaning (outliers), exploring data structure, and communicating information. In the 18th and 19th century, governments began using graphics to understand population and economic interests. Today, we see graphics everywhere from scientific journals to mass media in the newspapers, TV, and internet. Despite the popularity of graphics, we are too accepting of them as default without asking critical questions about the graphics we create or view. We must begin asking ourselves "How effective is this graph at communicating useful information?". We now have an infinitely many number design choices: colors, symbols, scales, limits.  There is a need for an established set of concepts and terminology to build their graphics from so they can actively choose which of many possible graphics to draw in order to ensure their charts are effective at communicating the intended result. This picture is a famous chart in astronomy that shows the life cycle of a star, classifying it by its brightness and color. It wasn’t originally intended to show the life cycle of a star, but to show the relationship between the two - and at the time, it wasn’t known that the star’s life cycle governed the relationship. But, when you look at the chart, it’s very clear that there is a relationship, and stars can take several different paths through the “life cycle” depending on their type.
...
-  13 data sets that have nearly identical statistical properties that appear very different when graphed
...
Another common way to display data is through numerical summary tables. So why would we need  to visualize it too? A popular example is the Datasaurus package in R which has 13 datasets that has nearly identical simple statistical properties that appear differently when graphed. This data can be used to demonstrate the importance of graphing data before it is analyzed. Each dataset has 142 xy pairs of data, where the mean, median, standard deviation, and correlation coefficient is almost identical for each data set.
...
I only chose six for space purposes, but as we can see here, each data set has the same mean, standard deviation, and correlation coefficient between x and y. So someone looking at this data might think that this data is pretty similar. However, visualizing it tells a different story. 
...
Here is the plots of the data from the previous slides. We can see here, that while the summary statistics is almost identical, the data looks very different when plotted. Hence, it is always important to visualize the given data to view and explore the patterns within the data.
...
As I mentioned previously, we are surrounded by visualization so it is important to accurately display the data. However, data is often displayed incorrectly or inaccurately. For example Cleveland (1984) found that 30% of the plots used in the journal Science, had some sort of error. Where error was defined as either incorrectly displaying the data or having ineffective design elements. This problem still persists to this day as in a more recent sample, Gordan and Finch (2015) categorized 40% of the 97 graphs they sampled as having an error (defined similarly as above). So more education needs to be made on how to make more effective visualizations.   
...
Effective graphics take advantage of how people's brains work. This can be divided into three categories, where we will talk more in-depth about each of them: preattentive processing, perceptual grouping, awareness of visual limitations. So we want to exploit how we know the brain work so users can focus on the data instead of deciphering the chart.  
...
Reduces the amount of work users of you graph have to do when view they view it

???

First, we want to take advantage of preattentive processing of the brain. Preattentive features are things that "jump out" immediately (notice right away - or in less than 500 ms). Some things that are preattentive include color, shape, angle, movement, and spatial location. Within these preattentive features, there is a hierarchy. For example color is stronger than shape (meaning it is more easily noticeable). Designing your graphic to utilize these features reduces the amount of work users have to do when they view your graph. Frees up their mind for other things. 
...
Once again, find the green circle? How long did it take you this time compared to last time?

You probably noticed the target in this picture first. Example of why color is in the hierarchy before shape. 
...
How to take advantage of brain to make graphs part two
...
Gestalt principles are the predictable ways by which we organize sensory information (how we interpret ambiguity in a visual scene). We will just touch on the main ones here that are highly used in visualization, but there are others. The first is proximity which says that things that are close together are likely part of the same unit. For example, the black boxes to connect the E are attached, so perceived as the letter. Next is the law of similarity that says that thing that are viewed as part of a group if they look similar (the two T's). The law of enclosure says that it is easier to interpret things if you imagine them as a closed figure (G). And finally the law of continuation says that figure with edges that are smooth are more likely to be continuous than things with edges that are sharp angles (we are more likely to perceive continuous, smooth flowing lines rather than jagged, broken lines - elements that are arranged on a line or curve are perceived to be more related than elements not on the line or curve.). 
...
```{r gestalt-hierarchy}
tibble("Gestalt Hierarchy" = c("Enclosure", "Continuity", "Proximitiy", "Similarity"),
...
This table shows some of the ways the gestalt principles can be implemented on a graph. For example, with the enclosure principle this can be implemented through facets (bring up datasaurus example - faceting by dataset).  For continuity, if you want items to be perceived as continuous, use a line (like a time series). We'll have example of proximity and similarity on the next slide, but for proximity can make use of white space (ie. more white space, items are more spread out). Similarity uses the same color or shape to denotes similar objects. So we can use this principles in practice because they let us know how we perceive groups and maybe why some group are more noticeable at first than other (think back to preattentive features with shape/color). Understanding these helps us to emphasize the important comparisons we need to make. 
...
Here's an example that utilizes the Gestalt principles of proximity and similarity (same data where groups are shown differently - once again shape vs color). The shape and color denote the similarity principle which are denotes by the shape/color to denote groups. Additionally for proximity, the data seemed to all be clumped together in the middle, so would expect all the data to have something in common.

Plot 1: proximity then similarity
Plot 2: the other way around
...
How to take advantage of brain to make graphs part three

Will talk about in the next slide, but not all graphical representations are equally accurate (like bar vs pie chart)

Unfortunately, there is relatively little research on other disabilities + statistical graphics, so much more work needs to be done in this area. Good to use alt-text for graphics (verbal description of image with each graphic so can explain what you should see). Utilize contrasts (maybe can't make out color but can see difference between light and dark). Textures and patterns can be hard to make out (like different lengths of lines). Additionally, will also talk about color in a couple of slides, and how to take into account those who are color-blind when using color to represent this. 
...
Cleveland and McGill (1984) want to identify elementary perceptual tasks that are carried out when people extract quantitative information from graphs, where they are ordered based on how accurately people extract the quantitative information from the graph (experiment). Ordered based on numerical accuracy. 

When you design a visualization, try to make the most important variables represented by dimensions that are more accurate.
...
The image above hear is ordered in some of the accuracy of numerical judgements. For example, length is higher up then angle, so always prefer bar chart over pie chart.
...
Here's an example of the same data displayed in both a pie chart and bar chart. Talk about the two greenish pieces of the pie chart and how people might have a hard time judging which one is bigger, and this judgement is made quicker and more accurate in the bar chart.  
...
+ Be conscious of what certain colors “mean”
    ...
    + It is also important to be conscious of the social baggage that certain color schemes may have - the pink/blue color scheme often used to denote gender can be unnecessarily polarizing.
...
There are packages such as `RColorBrewer` and `dichromat` that have color palettes which are aesthetically pleasing, and, in many cases, colorblind friendly (`dichromat` is better for that than `RColorBrewer`)

Quantiative color schemes - where can make use of common associations. Like blue and water. 

Lots of arguement on qualitative schemes (hard to make them color blind friendly, but have each color people equally spaced in the color space)
...
+ Utilize double encoding: use color and another aesthetic (line type, shape)
  ...
  .center[
```{r results='asis', echo = F, include = T, cache = T, eval = TRUE}
i1 <- img_modal(src = "images/dual-encode.png", alt = "Dual Encoding Example",other=list(width="25%"))
...
Not everyone perceives color the same way. Some individuals have color blindness or color deficiencies (link to take a test if you want to see for yourself). Some suggestions on how to keep this in mind:
...
Plot is an example of double encoding
...
Next let's talk about some of my actual research
...
???
Played with the transparency a bit more. Eventually would like to use the ggblend package, which is developing for the blending of objects. However, it doesn't work for polygon on top of polygon. Submit a bug report. Additionally, we introduced categorical groups for both variables. Trial Design was categorical to begin width. And I decided to use a categorial scale for yield to cut down on what needed to be blended. The breaks for yield were created using quantiles for the breaks. This has been shown in some research to be the best pereptually for cateogorical breaks.  
...
- Possibility of linked graphics
  - Show trial design plot, can then click on a trial plot to view the scatterplot between applied treatment and yield. 
...
print(bib[[c("searcy1997precision", "vanderplas2020", "Wang2018TowardsEC", "zhang_2008", "MIDWAY2020100141", "todorovic2008gestalt", "spacetime_wilke_2019", "macdonald_1999", "sar_2020", "schreyer_elastic_2006", "huang2009", "unwin2020data")]], 
      .opts = list(check.entries = FALSE, style = "html", bib.style = "authoryear")
      )
```
</font>
